...
public class FragmentMangerRequest extends Fragment {
    ...
    public FragmentMangerRequest() {
    }

    public FragmentMangerRequest(Activity context) {
    ...
    public class Adapter extends RecyclerView.Adapter<FragmentMangerRequest.Adapter.ViewHolder> {

        List<SignupUserDTO> signupUserDTOS;

        public Adapter(List<SignupUserDTO> signupUserDTOS) {
            this.signupUserDTOS = signupUserDTOS;
        }

        @NonNull
        @Override
        public FragmentMangerRequest.Adapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            ...
            return new FragmentMangerRequest.Adapter.ViewHolder(view);
        }
        ...
        @Override
        public void onBindViewHolder(@NonNull FragmentMangerRequest.Adapter.ViewHolder holder, final int position) {
            final Context innerContext = holder.itemView.getContext();
            Date registrationDate = signupUserDTOS.get(position).getRegistrationDate();
            ...
            holder.name.setText(signupUserDTOS.get(position).getWorkerName());
            holder.phoneNumber.setText(UtilityFunctions.getPhoneNumberInFormat(signupUserDTOS.get(position).getWorkerPhone()));
            picassoGetCircleImage(context, signupUserDTOS.get(position).getProfilePicture(), holder.profilePic, holder.profile_shimmer, R.drawable.side_profile_icon);

            holder.itemCardview.setOnClickListener(v -> {
                try {
                    ScreenDeskUserDetail.deskUserDetailsScreenDTO = signupUserDTOS.get(position);
