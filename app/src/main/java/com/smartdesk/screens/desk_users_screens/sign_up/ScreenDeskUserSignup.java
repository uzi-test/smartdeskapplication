...
public class ScreenDeskUserSignup extends AppCompatActivity {

    private Activity context;

    private PageStepIndicator indicator;
    private ViewPager viewPager;
    private PagerAdapterDeskUserSignUP _pagerAdapterDeskUserSignUP;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Constants.const_usersSignupDTO.setRole(Constants.deskUserRole);
        actionbar();
        UtilityFunctions.setupUI(findViewById(R.id.parent), context);
        initLoadingBarItems();
        nextBtn = findViewById(R.id.nextbtn);
        linearHide = findViewById(R.id.linearHide);
        _pagerAdapterDeskUserSignUP = new PagerAdapterDeskUserSignUP(this, getSupportFragmentManager());

        viewPager = findViewById(R.id.view_pager);
        indicator = findViewById(R.id.tabs);
        viewPager.setAdapter(_pagerAdapterDeskUserSignUP);
    ...
    private void actionbar() {
        ...
        ((TextView) findViewById(R.id.actionbarInclude).findViewById(R.id.actionTitleBar)).setText("Worker Registration");
    ...
    public void changeStateOnValid() {
        ...
        FragmentDeskUserStep1 fragmentDeskUserStep1 = (FragmentDeskUserStep1) getReferenceFragment(0);
        FragmentDeskUserStep2 fragmentDeskUserStep2 = (FragmentDeskUserStep2) getReferenceFragment(1);
        if (position == 0) {
            fragmentDeskUserStep1.validationsAndGetValues();
            if (isOkayStep1) {
                changePage(1);
            }
        } else if (position == 1) {
            fragmentDeskUserStep2.validationsAndGetValues();
            fragmentDeskUserStep1.validationsAndGetValues();
            if (!isOkayStep1)
                changePage(0);
            else if (!isOkayStep2)
                        ...
                        try {
                            if (task.isSuccessful()) {
                                if (!task.getResult().isEmpty()) {
                                    isOkayStep1 = false;
                                    fragmentDeskUserStep1.met_phoneNumber.setError("Phone Number already registered");
                                ...
                                queryCnic.get().addOnCompleteListener(querySnapshotTask -> {
                                    try {
                                        if (querySnapshotTask.isSuccessful()) {
                                            ((ScreenDeskUserSignup) context).stopAnim();
                                            if (!querySnapshotTask.getResult().isEmpty()) {
                                                isOkayStep1 = false;
                                                fragmentDeskUserStep1.et_email.setError("Email already registered");
    ...
    public void changePage(int page) {
        try {
            if (PagerAdapterDeskUserSignUP.pagesCount != 2) {
                PagerAdapterDeskUserSignUP.pagesCount = 2;
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == PermisionCode.MY_LOCATION_PERMISSIONS_CODE && resultCode == ((Activity) context).RESULT_OK) {
            System.out.println("AAAAAAAAAAAA GOES RESULT");
        }
    }
